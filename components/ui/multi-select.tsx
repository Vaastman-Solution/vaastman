...
export function MultiSelectContent({
  search = true,
  creatable,
  uppercaseSearch = false,
  children,
  ...props
}: {
  search?: boolean | { placeholder?: string; emptyMessage?: string };
  creatable?: boolean | { createLabel?: (value: string) => ReactNode };
  uppercaseSearch?: boolean;
  children: ReactNode;
} & Omit<ComponentPropsWithoutRef<typeof Command>, "children">) {
  const { items, selectedValues, toggleValue, onItemAdded } =
    useMultiSelectContext();
  ...
  const resolveExistingValue = (value: string) => {
  ...
  const existingSearchValue = resolveExistingValue(normalizedSearchValue);
  const canCreate =
    !!creatable &&
    normalizedSearchValue.length > 0 &&
    existingSearchValue == null;

  const createLabel =
    typeof creatable === "object" && creatable.createLabel
      ? creatable.createLabel(normalizedSearchValue)
      : `Add "${normalizedSearchValue}"`;

  const createOption = () => {
    if (!canCreate) return;
    onItemAdded(normalizedSearchValue, normalizedSearchValue);
    if (!selectedValues.has(normalizedSearchValue)) {
      toggleValue(normalizedSearchValue);
    }
    setSearchValue("");
  };

  const addValuesFromInput = (value: string) => {
    const parts = value.split(",").map(normalizeValue).filter(Boolean);
    if (!parts.length) return;

    parts.forEach((part) => {
      const existingValue = resolveExistingValue(part);
      if (existingValue != null) {
        if (!selectedValues.has(existingValue)) {
          toggleValue(existingValue);
        }
        return;
      }

      onItemAdded(part, part);
      if (!selectedValues.has(part)) {
        toggleValue(part);
      }
    });
            ...
            <CommandInput
              onKeyDown={(event) => {
                if (event.key === ",") {
                  ...
                  addValuesFromInput(searchValue);
                }
              }}
              onValueChange={(value) =>
                setSearchValue(uppercaseSearch ? value.toUpperCase() : value)
              }
              placeholder={
